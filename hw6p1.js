...
class PlayerSourceJson {
    ...
    async createPlayer(fname, lname, handed, initial_balance ) {
        ...
        let result;
        let player = {
            balance_usd_cents: Number(initial_balance)*100,
            created_at : new Date(),
            fname:fname,
            lname:lname,
        ...
        try{
            connect = await this.client.connect();
            let db = this.client.db(this.config.db);
            result = await db.collection(this.player).insertOne(player)
        }catch (error) {
            console.log(error)
...
app.get('/api/player', async (req, res) => {
    ...
    await player_data.getPlayers(active_status)
    .then((player) => {
        res.status(200).send(JSON.stringify(player))
    })
    .catch((err) =>res.sendStatus(404))
...
app.post('/api/player', async (req, res) => {
    ...
    if (
        ...
        isNaN(initial_balance_usd_cents) ||
        initial_balance_usd_cents < 0 ||
        !Number.isInteger(initial_balance_usd_cents)
    ) {
...
app.get('/player/create.html',async(req,res) => {
    const contents = fs.readFileSync(__dirname + '/public/player/create.html', 'utf8');
    res.send(contents);
})
...
app.get('/players.html', async (req, res) => {
    try {
      ...
      res.sendFile(__dirname +'/views/pages/players.html')
    //   , { players });
    } catch (err) {
        console.log(err)
...
app.get('/player/:id/edit.html' ,async(req,res) => {
    const contents = fs.readFileSync(__dirname + '/public/player/edit.html', 'utf8');
    res.send(contents);
})
